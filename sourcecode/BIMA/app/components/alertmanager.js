...
//
//
//This file is used only in development to test the feature
//Everythings in this file is already included in Staff Application
//
//
//
...
export default function AlertManager() {
  ...
  ReportPush = (topic,detail) => {
    firebase.database().ref('alertmanager/' + firebase.auth().currentUser.uid).child('alertid'+ alertid).set({staffid:firebase.auth().currentUser.displayName,topic,detail});
    Alert.alert(
  ...
  SendReport = (topic,detail) => {
    ...
    Alert.alert(
      ...
      [
        {text: 'Cancel', onPress: () => firebase.database().ref('alertmanager').child('alertcount').set(firebase.database.ServerValue.increment(-1)), style: 'cancel'},
