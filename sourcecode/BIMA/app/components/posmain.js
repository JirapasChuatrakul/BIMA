...
export default class POSMain extends Component {
  constructor(props){
    ...
    copyItem = () => {
      ...
      firebase.database().ref('history/' + firebase.auth().currentUser.uid).child('transaction'+ this.state.orderid).set({orderid:this.state.orderid, date: newDate, status: 'Successfully', totalprice: this.state.allprice});
      this.copyFirebaseObject(firebase.database().ref('cart/' + firebase.auth().currentUser.uid), 
                              firebase.database().ref('history/' + firebase.auth().currentUser.uid + '/' + 'transaction' + this.state.orderid + '/').child('itemlist'))
      firebase.database().ref('summary/' + firebase.auth().currentUser.uid + '/recentselling').remove()
      this.copyFirebaseObject(firebase.database().ref('cart/' + firebase.auth().currentUser.uid), 
                              firebase.database().ref('summary/' + firebase.auth().currentUser.uid).child('recentselling'))                        
